...
# ── Decoder-only forward (for precomputed embeddings) ────────────
def decoder_forward(sam, embeddings, bbox, device):
    """
    Run only prompt_encoder + mask_decoder on pre-computed embeddings.
    embeddings: (B, 256, 64, 64)
    bbox: (B, 4) in 1024 space
    Returns: mask logits (B, 1, 256, 256)
    """
...
# ── Train one epoch ──────────────────────────────────────────────
def train_epoch(sam, loader, optimizer, scheduler, scaler, device,
    ...
    for batch in loader:
        ...
        with torch.amp.autocast("cuda"):
            if precomputed:
                emb = batch["embedding"].to(device)  # (B, 256, 64, 64)
                mask_logits = decoder_forward(sam, emb, bbox, device)
            else:
...
def validate(sam, loader, device, max_batches: int = 0,
    ...
    for batch in loader:
        ...
        # Compute Dice
        pred = (torch.sigmoid(mask_logits.float()) > 0.5).float()
...
def main(target: str = "WT", epochs: int = None, batch_size: int = None,
         ...
         resume: bool = False, precomputed: bool = False):
    t0 = time.time()
    ensure_dirs()
    ...
    mode = "PRECOMPUTED" if precomputed else "FULL"
    print(f"[SAM+LoRA {target}] device={device}  epochs={epochs}  bs={batch_size}  lr={lr}  AMP=fp16  mode={mode}")
    ...
    # ── Data ─────────────────────────────────────────────────────
    if precomputed:
        train_ds = SAMPrecomputedDataset(
            split="train", target=target, jitter=True,
            jitter_shift=jitter_shift, jitter_scale=jitter_scale)
        val_ds = SAMPrecomputedDataset(
            split="val", target=target, jitter=False)
    else:
    ...
    nw = num_workers
    train_sampler = DatasetCaseGroupedSampler(train_ds.rows, seed=SEED)
    train_loader = DataLoader(
        train_ds, batch_size=batch_size, sampler=train_sampler,
        num_workers=nw, pin_memory=(nw > 0), drop_last=True,
        persistent_workers=(nw > 0))
    val_sampler = DatasetCaseGroupedSampler(val_ds.rows, seed=SEED)
        ...
        if not last_ckpt.exists():
            ...
            if "scheduler_state_dict" in ckpt_state:
                ...
                for _ in range(resumed_steps):
                    scheduler.step()
            if "scaler_state_dict" in ckpt_state:
                scaler.load_state_dict(ckpt_state["scaler_state_dict"])
...
if __name__ == "__main__":
    ...
    p.add_argument("--precomputed", action="store_true",
                   help="Use pre-computed encoder embeddings (10x faster)")
